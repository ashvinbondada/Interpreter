...
class ClassDef:
    ...
    def __check_method_names_and_types(self, method_def):
        ...
        ) or ((method_def.return_type.type_name in list(self.interpreter.class_index.keys())
               ) and (type(self.interpreter.class_index[method_def.return_type.type_name]) == TemplateClassDef)): #checks that return type isn't a defined type or void
            self.interpreter.error(
                ErrorType.TYPE_ERROR,
...
class TemplateClassDef:
    def __init__(self, class_def,interpreter):
        ...
        self.var_list = self.class_def[2]

        # self.createObjDef(['Square', 'Dog'])
 
    def createObjDef(self, var_replacements):
        # print(self.class_def)
        print(var_replacements)
        print(self.var_list)
        if len(self.var_list) != len(var_replacements):
            self.interpreter.error (
                ErrorType.TYPE_ERROR,
                "Expected template types when were passed",
                3 # line number
            )
        #create mapping and replace generic types
        template_to_actual_type = {self.var_list[i]: var_replacements[i] for i in range(len(var_replacements))}
        source = self.replace_template_types(template_to_actual_type, self.class_def)
        #converting to normal class
        source[0] = InterpreterBase.CLASS_DEF 
        # print('l',source)
        for typee in source[2]: 
            # print('b',typee)
            source[1] += '@' + typee
        
        # print(source)
        return ClassDef(
            [source[i] for i in range(len(source)) if i !=2]
            , self.interpreter)

    def replace_template_types(self, mapping, source):
        for i in range(len(source)):
            if isinstance(source[i], list):
                source[i] = self.replace_template_types(mapping, source[i])
            if '@' in source[i] and '"' not in source[i]:   # its a var
                type_split = source[i].split('@')
                source[i] = type_split[0]
                for term in type_split[1:]:
                    source[i] += '@' + mapping[term]
            elif '"' not in source[i]: 
                try: source[i] = mapping[source[i]]
                except: pass
        return source
