...
class ObjectDef:
    ...
    def __get_obj_with_method(self, start_obj, method_name, actual_params):
        # print('get_obj_with_method', method_name, actual_params)
        # if actual_params:
        #     print(actual_params[0][1].v)
        cur_obj = start_obj
    ...
    def call_method(self, method_name, actual_params, super_only, line_num_of_caller):
        ...
        # print(actual_params)
        # if actual_params and type(actual_params[0]) == tuple and actual_params[0][0] == ObjectDef.STATUS_EXCEPTION: 
        #     return actual_params[0]
        ...
        # print(method_name, status, return_value)
    ...
    def __execute_begin(self, env, return_type, code, has_vardef=False):
        ...
        for statement in code[code_start:]:
            ...
            if status in [ObjectDef.STATUS_RETURN, ObjectDef.STATUS_EXCEPTION]:
    ...
    def __add_locals_to_env(self, env, var_defs, line_number):
        for var_def in var_defs:
            ...
            var_name = var_def[1]
            if len(var_def) < 3:
                # if ('@' in var_type.type_name and 
                #     set(['int', 'bool', 'string']) ^ set(var_type.type_name.split('@')[1:])):
                #         self.interpreter.error(
            ...
            else: default_value = create_value(var_def[2])
    ...
    def __execute_print(self, env, code):
        ...
        for expr in code[1:]:
            ...
            if isinstance(term,tuple): return term
            val = term.value()
            typ = term.type()
    ...
    def __execute_if(self, env, return_type, code):
        ...
        if isinstance(condition, tuple): return condition
    ...
    def __execute_try(self, env, return_type, code):
        # print('__execute_try') 
        # env.block_nest()
        _, try_statement, catch_statement = code
        ...
        if status == ObjectDef.STATUS_EXCEPTION:
            ...
            env.set(InterpreterBase.EXCEPTION_VARIABLE_DEF, VariableDef(InterpreterBase.EXCEPTION_VARIABLE_DEF, return_value.type(),return_value))
            # print('in try',return_value.v, catch_statement)
            status, return_value = self.__execute_statement(env, return_type, catch_statement)
        # raise Exception ## stop
        return status, return_value

    def __execute_throw(self, env, code):
        # print('__execute_throw')
        _, error_str = code
        error_str = self.__evaluate_expression(env, error_str, code[0].line_num)
        if isinstance(error_str, tuple):  return error_str
        status = ObjectDef.STATUS_EXCEPTION # 2
        if error_str.type() == ObjectDef.STRING_TYPE_CONST: 
            return status, error_str
        else: InterpreterBase.error(self.interpreter, "ErrorType.TYPE_ERROR")
    ...
    def __evaluate_expression(self, env, expr, line_num_of_statement):
        ...
        if operator in self.binary_op_list:
            ...
            for op in [operand1, operand2]: # checking for exception
                if isinstance(op, tuple): return op
            if (
        ...
        if operator in self.unary_op_list:
            ...
            if isinstance(operand, tuple): return operand # throwing exception
    ...
    def __execute_call_aux(self, env, code, line_num_of_statement):
        ...
        if obj_name == InterpreterBase.ME_DEF:
            ...
            if isinstance(obj_val, tuple): return obj_val
            if obj_val.is_null():
                self.interpreter.error(
        ...
        actual_args = []
        for expr in code[3:]:
            res = self.__evaluate_expression(env, expr, line_num_of_statement)
            if isinstance(res, tuple): return res # case of error thrown
            actual_args.append(
                res
            )
